...
while(x) {
    ...
    instruction = intArray.slice(index, index+4);

    opAndMode = intcode.convertIntToString(instruction[0]);

    if (opAndMode.length >= 3) {
        modes = intcode.getModesArray(opAndMode);
        opcode = intcode.getOpcode(opAndMode);
    } else if (opAndMode.length === 1) {
        opcode = opAndMode;
    } 
    ...
    if (opcode === undefined || opcode === 0 || opcode > 8) {
        console.log('intArray: ' + intArray);
        throw new Error('opcode is ' + opcode);
    }

    modes.push(0,0,0,0);

    if (opcode === 1) {
        intArray = intcode.addNumbers(intArray, modes, index);
        index = index + 4;
    }

    if (opcode === 2) {
        intArray = intcode.multiplyNumbers(intArray, modes, index);
        index = index + 4;
    }

    if (opcode === 3) {
        intArray = intcode.inputValue(intArray, modes, index);
        index = index + 2;
    }

    if (opcode === 4) {
        intcode.outputValue(intArray, modes, index);
        index = index + 2;
    }

    if (opcode === 5) {
        index = intcode.jumpIfTrue(intArray, modes, index);
    }

    if (opcode === 6) {
        index = intcode.jumpIfFalse(intArray, modes, index);
    }

    if (opcode === 7) {
        intArray = intcode.lessThan(intArray, modes, index);
        index = index + 4;
    }

    if (opcode === 8) {
        intArray = intcode.equal(intArray, modes, index);
        index = index + 4;
    }

    if (intArray[index] === 99) {
        console.log('Done! 99');
        x = false;
    }
